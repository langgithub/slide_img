...
def get_logger(log_name="",
               log_path='/tmp/logs',
               single_log_file_size=1024 * 1024 * 1024 * 5,
               log_to_file=True,
               backup_count=1):
    """:return a logger"""
...
path = os.path.dirname(os.path.dirname(__file__)) + "/log"
# logger = get_logger("persona_server", path)
# logger = get_logger("persona_server1", path)
logger = get_logger("persona_server2", path)
logger_schedule = get_logger("schedule", path)
